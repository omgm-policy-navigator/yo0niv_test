...
| pytest | `cd backend && UV_CACHE_DIR=../.uv-cache uv run pytest` | Passed | 10 passed, 1 Starlette/httpx deprecation warning |
| ruff | `cd backend && UV_CACHE_DIR=../.uv-cache uv run ruff check app tests` | Passed | All checks passed |
| production image | `docker build -t yo0niv-test-backend-runtime ./backend` | Passed | Multi-stage runtime image builds without dev/test files |
...
| compose pytest | `docker compose run --rm backend pytest` | Passed | 10 passed, 1 Starlette/httpx deprecation warning |
...
| compose DB health | `curl -i http://127.0.0.1:8001/health/db` | Passed | 200 OK, PostgreSQL `SELECT 1` |
